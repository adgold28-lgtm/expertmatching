...
5. H-11: `export const maxDuration = 300` on the sourcing worker; `publishSourcingJob` sends `Upstash-Deduplication-Id: sourcing-<projectId>-<sourcingStartedAt>` (hyphens; QStash rejects ':') and the job body carries `runId = sourcingStartedAt`; `runSourcingJob` refuses to write when the project's `sourcingStartedAt !== job.runId` or `sourcingStatus !== 'running'` (conditional via `updateProjectFields` preconditions; if the store cannot express it, read-check then write and document the small window). Two concurrent starts (M TOCTOU) are closed the same way: the start route sets `running` with a conditional update where status is not running.
