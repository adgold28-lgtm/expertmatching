...
- [ ] **Founder decision: Calendly, remove or fix.** Probed 2026-09-09: `api.calendly.com` answers 401 to every unauthenticated call, so every Calendly link yields no slots and looks like no connection. `probeCalendlyLink()` is written and unwired for a connect-time refusal. Recommendation: remove.
