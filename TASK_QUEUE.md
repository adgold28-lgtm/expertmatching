...
Last updated: 2026-09-10 (session 10)

## NOW (blocking or broken)
- [ ] 2026-09-10 FOUNDER, BEFORE MERGING the Stripe review branch: in Stripe → Developers → Webhooks add `checkout.session.async_payment_succeeded` (test AND live). Delayed-notification payments now require it — without it a payment that clears late is never recorded. (Still outstanding alongside it: `account.updated` on connected accounts.)
- [x] 2026-09-10 STRIPE REVIEW ROUND 2 — the independent Codex/ChatGPT review's findings (R1–R5), re-verified against this code and fixed on `claude/stripe-security-review-ef3yjx`; see SECURITY_AUDIT.md "Second pass". R1 failed webhook handlers answered 200 so Stripe never retried; R2 `checkout.session.completed` was treated as payment (delayed methods complete unpaid); R3 the payout retry sweep never checked the client paid; R4 `pending` accounts were not gated by middleware; R5 two auth verifications per request. Also fixed three defects IN those fixes: F1 permanent failures became a 3-day retry loop risking endpoint disablement, F2 a blocked-but-paid payout became invisible to the sweeps, F3 the status gate trapped blocked users with no way to sign out. Open: R6 dependency advisories, R7 event ordering, refunds unmodelled, no browser error capture.
- [x] 2026-09-10 STRIPE / BILLING SECURITY REVIEW — branch `claude/stripe-security-review-ef3yjx`, three fixes, see SECURITY_AUDIT.md "Stripe / Billing Review (September 2026)". S-1 (HIGH): the project owner — the person whose card is charged — could `PUT { paymentStatus: 'paid' }` on their own engagement and walk past createAndSendInvoice's double-bill guard uncharged; the seven payment/duration fields are now `SERVER_OWNED_FIELDS` (lib/expertWriteFields.ts) and answer 403. S-2: the rate lock only covered `clientRate`, so `expertRate` could rewrite a settled engagement's price. S-3: the Zoom `meeting.ended` auto-charge had no duration ceiling (and produced NaN on a bad `start_time`); `billableCallMinutesFromWindow` bounds it to MAX_BILLABLE_MINUTES, now shared with POST …/complete. `npx tsx scripts/test-billing-boundaries.ts` (48 checks) + build:local clean. Nothing in the product sent those fields, so no UI change.
