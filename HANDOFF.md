...
- **Local build on this Mac:** `npm run build` fails downloading Google Fonts (ETIMEDOUT in Next's bundled fetcher). Workaround: `NEXT_FONT_GOOGLE_MOCKED_RESPONSES=<path to a font-mocks.js>` or build in a `git worktree` with `node_modules` symlinked. Never build while `next dev` runs (shared `.next`).
