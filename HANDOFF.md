...
## Session 12 (2026-09-14/15): the Structured Request & Screening Flow. Branch `claude/laughing-dirac-cs2b96`, pushed.

**What it is.** A second front door beside Brief → Matches → Conversations: a client writes a topic plus 3–6 learning objectives (a *request*), each objective becomes a first-person yes/no *stem* and a role-and-timeframe *proof prompt*, the client edits and approves the set, staff mint one single-use signed link per candidate, the expert answers every stem on a phone in under 90 seconds, and the client sees coverage (yes / total) and the expert's own sentences before any call is booked. The thesis and every decision are in `docs/SCREENING_FLOW_PLAN.md`; read it before touching the flow.

**Commits, one per build step:** plan + shell (`2c968fe`), schema/types/token/store/validation (`12d9222`), intake + list + `GET|POST /api/requests` (`4c3c169`), LLM conversion + approval screen (`953e11d`), signed links + the expert form (`fa9b490`), and the review table + actions + docs (this commit).

**Founder review, 2026-09-15.** Tables renamed `requests` → `expert_requests` and `outreach_tokens` → `screening_tokens` (API paths unchanged). The client-side rate is a required intake field with no default. `lib/screeningItems.ts` refuses proof prompts that solicit findings, outcomes, numbers, results, opinions or recommendations, regenerates at most twice, then uses the fixed string "In one sentence: what was your role in this and when?"; `scripts/test-screening-items.ts` drives twelve adversarial objectives through it offline.

**Where things live.** `supabase/migrations/20260914000000_screening_requests.sql` (five tables, RLS on, zero policies, stage-5 queries in the header); `lib/requestStore.ts` (Supabase + in-memory dev store); `lib/screeningToken.ts` (HMAC purpose `screening`, expires at the request deadline, revocable by hash); `lib/screeningItems.ts` (one `claude-opus-5` call, `proofPromptViolation` / `stemViolation`, one regeneration round, deterministic fallback); `lib/screeningView.ts` (the client-side redaction boundary: name, email and the expert-side ask are admin-only); `lib/screeningPublic.ts` (the expert-side boundary: topic, expert-side rate, call length, firm phrase, deadline, the questions — nothing else); `lib/screeningEmail.ts` (Resend, DISABLE_EMAILS, suppression fail-closed, CAN-SPAM footer). Routes under `app/api/requests/**` and the public `app/api/s/[token]`; pages `/requests`, `/requests/new`, `/requests/[id]`, `/s/[token]`; components under `components/requests/` plus `components/ScreeningForm.tsx`. Middleware opens `/s/` and `/api/s/` to the token gate.

**Gate.** `npx tsc --noEmit` clean; `npm run build:local` green after every step; new offline suites `test-screening-core` 248, `test-screening-items` 216, `test-screening-form` 73, `test-screening-redaction` 60, and the existing suites unchanged; `scripts/e2e-screening.ts` (intake → fallback generation → refused and accepted edits → approve → mint → public read → refused partial → full submission → single use → review payload → revoke → request-call → outcomes) green against `DISABLE_EMAILS=true npx next dev -p 3100` with `APP_AUTH_ENABLED` unset, which is the in-memory store. NOT exercised: the Supabase store against a real database (the SQL was applied and re-applied on a throwaway local Postgres 16 during the build, but the TypeScript store has only run in memory), the model call (stubbed in the items suite), a browser pass on a phone.

**Founder to do.** Paste the migration before deploying (the code does not tolerate the tables being absent). Decide the four items listed under NOW in TASK_QUEUE (table names, who mints, name-versus-background visibility, the SDK pin). Set nothing new in Vercel: the flow reuses `AVAILABILITY_TOKEN_SECRET`, `ANTRHOPICKEYREAL`, `RESEND_API_KEY`, `OUTREACH_FROM_EMAIL` and the base-URL variables.
