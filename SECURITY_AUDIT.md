...
**Fix:** Added the Anthropic key to `REQUIRED_VARS` in `lib/validateEnv.ts` and documented it in `.env.example`.  
**Update (issue #26):** The misspelled legacy variable `ANTRHOPICKEYREAL` has since been renamed to the standard `ANTHROPIC_API_KEY` across `generate-experts/route.ts`, `demo-readiness/route.ts`, `validateEnv.ts`, and `.env.example`. The old name is no longer referenced anywhere. **Deployments must rename the environment variable `ANTRHOPICKEYREAL` → `ANTHROPIC_API_KEY` or the app will fail startup validation.**
