...
| `requests` | A structured research request: topic + 3–6 learning objectives (docs/SCREENING_FLOW_PLAN.md) | `organization_id`, `owner_id`, `status` (`draft` → `approved` → `closed`), `topic_statement`, `targeting` jsonb, `call_count`, `deadline` (link expiry), `client_rate` (CLIENT-side), `call_length_min` | none | `draft` → `approved` |
