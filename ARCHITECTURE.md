...
| `expert_requests` | A structured research request: topic + 3–6 learning objectives (docs/SCREENING_FLOW_PLAN.md) | `organization_id`, `owner_id`, `status` (`draft` → `approved` → `closed`), `topic_statement`, `targeting` jsonb, `call_count`, `deadline` (link expiry), `client_rate` (CLIENT-side), `call_length_min` | none | `draft` → `approved` |
| `objectives` | One learning objective and the screening item it became | `request_id`, `position`, `objective_text`, `stem`, `proof_prompt`, `model_stem` / `model_proof_prompt`, `client_edited`, `source` | none | generated → edited → frozen at approval |
| `screening_tokens` | One screening link = one candidate on one request (NOT the Matchy reply token in lib/outreachToken.ts) | `request_id`, `expert_id` (text, cross-request key), `expert_email` (staff-only), `expert_snapshot` jsonb, `token_hash` (unique; raw token never stored), `expires_at`, `submitted_at` (single use), `revoked_at`, `call_requested_at`, `rate_accepted`, `rate_ask` (EXPERT-side, staff-only), `availability` | none | minted → submitted → call requested |
| `screening_responses` | One answer per (link, objective): yes / no / unsure + the expert's own sentence behind a yes | `token_id`, `objective_id`, `request_id`, `expert_id`, `answer`, `proof_text` | none | append |
| `call_outcomes` | Stage 5: what the call delivered per objective | `token_id`, `objective_id`, `expert_id`, `outcome` (`answered` / `partial` / `unanswered`), `marked_by` | none | upsert on (token, objective) |
...
**Calendly is hidden behind `CALENDLY_ENABLED` (Wave 5, founder decision 7).** Probed 2026-09-09: `api.calendly.com` answers 401 to every unauthenticated request and there is no Calendly credential in the app, so every link yields no slots. With the flag unset or not `'true'` (`lib/calendlyFlag.calendlyEnabled`), the onboarding and settings UI never offer Calendly, `POST /api/onboarding/calendar` answers `400 calendly_disabled` for `provider: 'calendly'`, and `connectionIsUsable` / `getClientSlotsForUser` treat an existing Calendly row as not connected, so the user is asked for Google or typed hours. `lib/fetchCalendlySlots.ts` is untouched so the provider can be switched back on once a real Calendly integration exists.

**Cancelling a booked call (Wave 5).** `docs/CALL_POLICIES_DRAFT.md` carries the approved policy; `lib/callPolicies.ts` is its pure form and `scripts/test-call-policies.ts` its proof. Every window compares server time with `booking.startUtc`: more than 24 hours out either side cancels free, inside 24 hours a cancel is *late* (exactly 24 h is free, one millisecond inside is late, an unparseable start is late because money fails closed). `lib/bookCall.cancelCall` takes a Redis `SET NX` lock (`booking-lock:<project>:<expert>`, 60 s, fail open like the intro lock) and then a `projectStore.mutateExpert` compare-and-set that refuses unless a live, uncancelled booking exists; the one write stamps `booking.cancelledAt/cancelledBy/cancelReason/lateCancel`, moves the engagement to `rejected_after_outreach`, clears `nudges` and blanks `scheduling.pickTokenHash` (so the picker link dies). Only after that write do the side effects run: Zoom delete, two per-recipient ICS `METHOD:CANCEL` copies (same UID, SEQUENCE+1) through `sendBookingEmail`, one Matchy thread line, a `call_cancelled` event (`{ by, late }`; the check constraint is widened by migration `20260910000000`, and the insert is tolerated failing until it is applied), and then either the money or the removal. Client: owner-only `GET`/`POST /api/projects/[projectId]/experts/[expertId]/booking/cancel`; the GET returns `{ window, fee }` for the two-step confirm in `ConversationThread`, and a late POST without `confirmLate: true` is refused `409 late_not_confirmed` with the fee. Expert: `POST /api/schedule/[token] { action: 'cancel', confirmLate? }`, token-gated like `pick`; the picker warns that a late cancel removes them from ExpertMatch. Cancel ends the booking; "find another time" is a separate action, and today `propose-times` refuses a cancelled engagement (`not_ready_to_schedule` / `nothing_booked`), so a fresh bookmark is the honest path until `READY_TO_SCHEDULE` admits a cancelled row. A staff cancel (`by: 'staff'`) is supported by the library and has no route yet.

**Who pays when a call does not happen.** `cancelOutcome(by, window)`: a client late cancel or client no-show charges 15 minutes at the client rate and pays 15 minutes at the expert rate (`lib/lateCancelBilling.applyClientLateCancelMoney`, keyed on `${icsUid}:late-cancel` so the per-call billing and payout guards treat it as its own call, and `runExpertPayout` derives 15 minutes from that suffix rather than the booked duration); an expert late cancel or expert no-show charges nothing, pays nothing, and runs `lib/expertRemoval.removeExpertForFault`: the expert's `contactEmail` goes on `outreach_suppressions` (reason `manual`, source project), the client gets a Matchy apology naming no client details, and `expertRemovedAt/expertRemovedFor` are stamped (that stamp, not the terminal status, is the idempotence key, because `cancelCall` has already written the status when it calls it). Attendance comes from Zoom `meeting.participant_joined` (`zoomAttendance`, matched on the participant's email, host role as a fallback for the expert). `meeting.ended` bills the ordinary call only when `resolveAttendance` says `both`; `client_no_show` and `expert_no_show` take the branches above; `unknown` (no telemetry) writes `attendanceReviewPending`, records `invoice/attendance_unconfirmed` and charges nothing, and `POST /api/admin/attendance { outcome }` is the staff decision that runs the identical `applyAttendanceOutcome`. **The Zoom app must subscribe to `meeting.participant_joined` before launch, or every call parks for review.**
...
**Champion-only first card (Wave 5).** The plain first-time `POST /api/onboarding/billing` now requires `org_role === 'org_admin'` or a platform admin, the gate `replace` and `activate` already carried; a member is answered `403 { error: 'champion_required', championEmail }` after the `alreadyComplete` short-circuit (so a firm with a card on file is unaffected) and `BillingStep` says "ask your champion", continuing into the walkthrough-limited product. `PATCH /api/org/members { action: 'transfer_champion', email }` promotes the target and demotes the sitting champion in one request (`app/api/org/members/championTransfer.ts`, pure and tested), refuses to leave zero org admins, and is what the Team page's "Make champion" calls.

**Refund clawback is a staff action, never automatic.** `charge.refunded` and `charge.dispute.created` still only mark `refunded` and alert. `POST /api/admin/payouts/reverse { projectId, expertId, reason }` (adminGuard) calls `stripe.transfers.createReversal` through `lib/stripeConnect.reverseExpertPayout`, idempotent on `expertPayoutReversedAt` and keyed on the payout's own idempotency key, and stamps `stripeTransferReversalId`; the control sits on the Needs Attention item the refund raises.
...
| `CALENDLY_ENABLED` | `'true'` re-enables the Calendly calendar option; unset or anything else hides it in the UI, refuses it on the connect route and treats an existing Calendly row as not connected (Wave 5). |
